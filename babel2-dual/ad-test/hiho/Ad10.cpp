...
namespace {
	...
	namespace math {
		...
		std::pmr::list<Expression> Expression::expressions = std::pmr::list<Expression>(&mr);
...
void hiho::ad10(double s, double sigma, double k, double r, double t, int simulation)
{
	Real ss{ s };
	...
	std::cout << "ad09 diff : " << diff << ", time : " << timer.duration() << " msec, delta : " << timer.value.d(ss) << std::endl;
